...
def train(args, io):
    ...
    for epoch in range(args.epochs):
        ...
        for data, label, seg in train_loader:
            ...
            ce_loss = criterion(seg_pred.view(-1, seg_num_all), seg.view(-1, 1).squeeze())
            if dice!=None:
                smooth = 1e-5
                pred_probs = torch.softmax(seg_pred.view(-1, seg_num_all), dim=1)
                pred_binary = (pred_probs[:, 1:].argmax(dim=1) > 0.5).float()
                target=seg.view(-1,1).squeeze()
                target_binary = (target == 1).float()  # 二值化处理
