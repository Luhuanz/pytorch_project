...
def update_tag_scheme(sentences, tag_scheme):
    ...
    for i, s in tqdm(enumerate(sentences)):
        tags = [w[-1] for w in s]
        if not data_utils.check_bio(tags): # 修正编码错误
...
def load_model_dataset(config):
    ...
    if not os.path.isfile(config.map_file):
        ...
        #通过调用一个预训练的词嵌入文件来增强训练数据集中的词汇表，同时也将测试数据集中出现的单词添加到词汇表中。
    ...
    if os.path.exists(config.modeldatasetpkl):
        ...
        train_data = prepare_dataset(train_sentences, word_to_id, tag_to_id)

        dev_data = prepare_dataset(dev_sentences, word_to_id, tag_to_id)

        test_data = prepare_dataset(test_sentences, word_to_id, tag_to_id)
        modeldatasetpkl = {}
