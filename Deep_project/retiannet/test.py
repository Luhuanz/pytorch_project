...
# 修改`retinanet/dataloader.py`文件中的39行`self.coco= COCO(os.path.join(self.root_dir, 'annotations', 'instances_' + self.set_name + '.json'))`，注释掉;
# 接着去掉40行的注释`# self.coco      = COCO('coco/annotations/image_info_test2017.json')`。
import numpy as np
import torchvision
import time
import os
