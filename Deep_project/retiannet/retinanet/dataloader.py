from __future__ import print_function, division # 兼容pythop2
import sys
import os
import torch
import numpy as np
...
from torch.utils.data import Dataset, DataLoader  # 封装数据
from torchvision import transforms, utils  # 翻转与预处理
from torch.utils.data.sampler import Sampler

from pycocotools.coco import COCO  #用来获取coco文件信息

import skimage.io  # 查看图片画图的一些包
...
#annotations是一个列表（元素个数对应数据集中所有标注的目标个数，注意不是图像的张数）
class CocoDataset(Dataset):
    """Coco dataset."""

    def __init__(self, root_dir, set_name='val2017', transform=None):
        """
        Args:
        ...
        self.transform = transform    # 对于每一个图片，设定某个或某些变换  normz aumg reizer
        ...
        # self.coco      = COCO('coco/annotations/image_info_test2017.json')  # 训练集图片的信息
        self.image_ids = self.coco.getImgIds() # 获得所有图片所对应的ID信息   因为 通过id可以知道图片的类别 每张图片的id

        self.load_classes()

    def load_classes(self):
        # load class names (name -> label) 生成名字到数字的对照表
        categories = self.coco.loadCats(self.coco.getCatIds())# 80类别不含前景 获得总的categroy数，80个和对应的名称
        categories.sort(key=lambda x: x['id']) # 按照id排序 1-全部图片

        self.classes             = {}   # person :0
        self.coco_labels         = {}
        self.coco_labels_inverse = {}
        for c in categories: #拿到每个类别的id
            self.coco_labels[len(self.classes)] = c['id']  # c['id] 为id  person类的全部id   全部类的id
            self.coco_labels_inverse[c['id']] = len(self.classes)
            self.classes[c['name']] = len(self.classes) #超类加入 80
