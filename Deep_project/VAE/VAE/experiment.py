...
#在每个训练步骤中如何计算损失并进行反向传播。
        ...
        results = self.forward(real_img, labels = labels) #torch.Size([128, 3, 64, 64]) torch.Size([128, 3, 64, 64]) torch.Size([128, 128])
#表示在损失函数中赋予 Kullback-Leibler 散度（KLD）项的权重。KLD 项是衡量模型预测值分布和实际目标值分布之间差异的一种指标。
