...
class T2T_module(nn.Module):
    ...
    def __init__(self, img_size=224, tokens_type='performer', in_chans=3, embed_dim=768, token_dim=64):
        ...
        if tokens_type == 'transformer':
           ...
           # print('adopt performer encoder for tokens-to-token')
            ...
            self.attention1 = Token_performer(dim=in_chans*7*7, in_dim=token_dim, kernel_ratio=0.5) # 3*7*7   147 64 0.5
            self.attention2 = Token_performer(dim=token_dim*3*3, in_dim=token_dim, kernel_ratio=0.5)  #64*3*3  576 64 0.5
            self.project = nn.Linear(token_dim * 3 * 3, embed_dim)
    ...
    def forward(self, x):
        ...
        x = self.attention1(x)  # attention 后dim 减少  2,3136. 64
        # print(x.shape)
        # exit()
        B, new_HW, C = x.shape
        x = x.transpose(1,2).reshape(B, C, int(np.sqrt(new_HW)), int(np.sqrt(new_HW)))#orch.Size([2, 64, 56, 56])
        # print(x.shape) torch.Size([2, 64, 56, 56])
        # exit()
        # iteration1: soft split
        x = self.soft_split1(x).transpose(1, 2) # c猜测 2,  348   3x3xC->576  torch.Size([2, 784, 576])

        # iteration2: re-structurization/reconstruction
        x = self.attention2(x)  # 猜测 2,, 576, 64  torch.Size([2, 784, 64])
        # print(x.shape)
        # exit()
        B, new_HW, C = x.shape
        x = x.transpose(1, 2).reshape(B, C, int(np.sqrt(new_HW)), int(np.sqrt(new_HW))) #torch.Size([2, 64, 28, 28])
        # print(x.shape)
        # exit()
        # iteration2: soft split
        x = self.soft_split2(x).transpose(1, 2)# 猜测 2，      3x3x64   2,196,576
        # print(x.shape)
        # exit()
        # final tokens
        x = self.project(x)#torch.Size([2, 196, 768])
        return x
...
class T2T_ViT(nn.Module):
    ...
    def forward_features(self, x):
        B = x.shape[0]
        x = self.tokens_to_token(x) #torch.Size([2, 196, 768])
        # print(x.shape)
        # exit()
        ...
        x = torch.cat((cls_tokens, x), dim=1) #torch.Size([2, 197, 768])
        x = x + self.pos_embed # 升级版正余弦#torch.Size([2, 197, 768])
        x = self.pos_drop(x)

        for blk in self.blocks:
            x = blk(x)
           # print(x.shape)  # 每个出来的dim都没变
    ...
    def forward(self, x):
        x = self.forward_features(x) #torch.Size([2, 768])

        x = self.head(x) #torch.Size([2, 1000])
