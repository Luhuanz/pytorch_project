...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, outer_dim=768, inner_dim=24, inner_stride=4):
        ...
        self.inner_dim = inner_dim #48 因为传入； 48

        self.num_words = math.ceil(patch_size[0] / inner_stride) * math.ceil(patch_size[1] / inner_stride) #16
    ...
    def forward(self, x):
        ...
        x = self.unfold(x) # B, Ck2, N torch.Size([1, 768, 196])
        x = x.transpose(1, 2).reshape(B * self.num_patches, C, *self.patch_size) # B*N, C, 16, 16  torch.Size([196, 3, 16, 16])
# 做 内部的wordembedding

        x = self.proj(x) # B*N, C, 8, 8 torch.Size([196, 48, 4, 4])  -> 内部信息融合用conv2D 论文里没这么写

        x = x.reshape(B * self.num_patches, self.inner_dim, -1).transpose(1, 2) # B*N, 8*8, C torch.Size([196, 16, 48])  4x4一个word
...
class TNT(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, outer_dim=768, inner_dim=48,
        ...
        self.proj_norm1 = norm_layer(num_words * inner_dim)  #16*48
        self.proj = nn.Linear(num_words * inner_dim, outer_dim) #16*48 768 768
        self.proj_norm2 = norm_layer(outer_dim)
