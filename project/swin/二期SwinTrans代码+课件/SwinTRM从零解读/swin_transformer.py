...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        shortcut = x
        x = self.norm1(x)
        x = x.view(B, H, W, C)## 从1 3136 96 转为  1 56 56 96  注意这个输入的一维那种向量转为了特征图，也就是一维3136，到了一个二维特征图 56 56 ，对应到原始图片是224 224时候就从
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.proj(x).flatten(2).transpose(1, 2)  # B Ph*Pw C  1 3136 96  3136就是我们的56*56 压扁  [1,3136,96]
        # print(x.shape)
