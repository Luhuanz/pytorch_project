...
class SwinTransformerBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # cyclic shift
        if self.shift_size > 0:
            #shift_size > 0时为SW-MSA
            shifted_x = torch.roll(x, shifts=(-self.shift_size, -self.shift_size), dims=(1, 2))
        else:
            #shift_size == 0时为W-MSA, 此时x不需要roll
            shifted_x = x

        # partition windows，切分window，将x的shape调整到理想的shape。
        # B, num_patches=h*w, C
        x_windows = window_partition(shifted_x, self.window_size)  # nW*B, window_size, window_size, C
        x_windows = x_windows.view(-1, self.window_size * self.window_size, C)  # nW*B, window_size*window_size, C

        # W-MSA/SW-MSA
        # SW-MSA时，需要mask一部分attn_map上的值，所以需要传入mask。
...
class PatchMerging(nn.Module):
    ...
    def forward(self, x):
        ...
        x0 = x[:, 0::2, 0::2, :]  # B H/2 W/2 C 起始 0，0
        x1 = x[:, 1::2, 0::2, :]  # B H/2 W/2 C 起始 1，0
        x2 = x[:, 0::2, 1::2, :]  # B H/2 W/2 C 起始 0，1
        x3 = x[:, 1::2, 1::2, :]  # B H/2 W/2 C 起始 1，1
    ...
    def forward(self, x):
        for blk in self.blocks:
            if self.use_checkpoint:
                ...
                x = blk(x)
        #不是最后一个stage时，需要调用下采样方法，这里这个方法是patch merging。
